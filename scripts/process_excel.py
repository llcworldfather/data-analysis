# -*- coding: utf-8 -*-
"""
BOM 成本分析脚本（Polars 版本）
依赖：pandas, openpyxl, polars（见 requirements.txt）
读取 data/ 下所有 .xlsx，全程以 Polars 高性能处理，结果写入 output/。

输出文件：
  默认（合并）：bom_analysis_<时间戳>.xlsx —— 5 张工作表
  --per-file：每个源文件一份 bom_analysis_<源文件名净化>_<时间戳>.xlsx

原始表必需列与结果表各列业务含义：见项目内 .cursor/rules/excel-bom-assistant.mdc「表格列说明」。
"""
...
def _print_no_excel_error() -> None:
    ...
    print(
        ...
        f"{listing}\n"
        f"请确认：① Cursor 打开的是**放了表的那个项目文件夹**（例如「副本」与原版是两套路径）；"
        f"② 文件在**项目根目录下的 data** 里，且勿放在子文件夹。"
    )
...
def _write_sheet(writer: pd.ExcelWriter, df: pl.DataFrame, name: str) -> None:
    ...
    pdf = df.to_pandas()
    # 将编码类列（整数或字符串形式的纯数字）转为字符串，避免 Excel 科学计数法
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="读取 data/ 下 Excel，生成 BOM 成本分析报表。"
    )
    parser.add_argument(
        "--per-file",
        action="store_true",
